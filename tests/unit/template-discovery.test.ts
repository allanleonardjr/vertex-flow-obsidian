...
describe("discoverVaultTemplates", () => {
  it("discovers a valid vault template as a WorkspaceTemplate", async () => {
    ...
    expect(
      viewRow.values.every(
        (v, i) =>
          v.icon ===
          snapshot.views.filter((w) => !isSystemViewId(w.id))[i].icon,
      ),
    ).toBe(true);
    ...
    expect(
      found[0].settings
        ...
        ?.values.every((v) => v.icon === undefined),
    ).toBe(true);
  });
  ...
  it("lets a built-in template win an id collision", async () => {
    const warnings: string[] = [];
    const io = fakeIo({
      "blank-workspace.md": templateSource("blank-workspace"),
    });
    const found = await discoverVaultTemplates(io, (m) => warnings.push(m));
  ...
  it("orders vault templates by createdAt, newest first", async () => {
    ...
    const newer = serializeTemplateMarkdown({
      meta: {
        id: "newer",
        name: "newer",
        createdAt: "2026-09-21T16:54:00.000Z",
      },
      workspace: snapshot.workspace,
      views: snapshot.views.filter((v) => !isSystemViewId(v.id)),
      dashboards: snapshot.dashboards,
  ...
  it("still discovers templates in the legacy Templates/ folder", async () => {
    ...
    expect(found[0].path).toBe(
      `${LEGACY_WORKSPACE_TEMPLATES_FOLDER}/legacy.md`,
    );
  });
