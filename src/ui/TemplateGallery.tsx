...
import {
  ...
  type VaultTemplate,
} from "../obsidian/template-discovery";
...
function Gallery({
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      const index = active instanceof HTMLElement ? cards.indexOf(active) : -1;
      ...
      if (event.key === "ArrowDown" || event.key === "j") moveTo(from + cols);
  ...
  return (
    <div className="vf-template-gallery">
      ...
      {vaultTemplates.length > 0 && (
        <>
          <h2 className="vf-template-section-title">
            Your templates{" "}
            <span className="vf-template-section-badge">Local</span>
          </h2>
      ...
      <h2 className="vf-template-section-title vf-template-section-gap">
        Built-in templates
      </h2>
...
function TemplateCard({
  ...
  return (
    <div
      ...
      className={`vf-template-card${template.path ? " vf-template-card-local" : ""}`}
      role="button"
      tabIndex={0}
    ...
    >
      {template.path && (
        <div className="vf-template-card-badge-row">
          <span className="vf-template-card-badge">Your template</span>
        </div>
      )}
...
function ConfigStep({
  ...
  const takenPrefixes = useMemo(
    () =>
      new Set(plugin.index.takenPrefixes().map((p) => p.trim().toUpperCase())),
    [plugin],
  );
  ...
  const prefixTaken =
    prefix.trim().length > 0 && takenPrefixes.has(prefix.trim().toUpperCase());
  ...
  const parentMissing =
    parent.length > 0 && plugin.app.vault.getAbstractFileByPath(parent) == null;
      ...
      <label className="vf-field">
        ...
        {prefixTaken ? (
          ...
          <small>
            ...
            <code>{prefix.trim() || "TSK"}-0001.md</code>. Must be unique across
            the whole vault.
          </small>
        )}
      </label>
