...
function canRevealInFolder(app: { showInFolder?: (path: string) => void }): boolean {
...
export function ExportDialog({
	...
	const [fields, setFields] = useState<Set<FieldId>>(() => new Set(DEFAULT_FIELDS));
	...
	const scope = useMemo<ExportScope>(() => {
		...
		if (scopeKind === "project") {
			const project = snapshot.projects.find((p) => p.path === scopeProjectPath);
			if (project) return { kind: "project", project };
		}
		return { kind: "workspace" };
	}, [lockScope, initialScope, scopeKind, snapshot, scopeViewId, scopeProjectPath]);
	...
	const revealInFolder = (file: TFile) => {
		const app = plugin.app as unknown as { showInFolder?: (path: string) => void };
	...
	const showReveal = canRevealInFolder(plugin.app as unknown as { showInFolder?: (path: string) => void });
					...
					<>
						<div className="vf-dialog-body">
							...
							<p className="vf-dialog-hint">
								This file type may not appear in Obsidian's own file list
								— turn on “Detect all file extensions” in Settings → Files
								and Links to see it in the File Explorer.
							</p>
						...
						{mode === "tasks" ? (
							<>
								<div className="vf-dialog-body">
									...
									<label className="vf-export-check">
										<input
											type="checkbox"
										...
										{FIELD_GROUPS.map((group) => {
											...
											return (
												<div key={group.id} className="vf-export-group">
													...
													{groupFields.map((id) => (
														<label key={id} className="vf-export-check">
															<input
																type="checkbox"
																checked={fields.has(id)}
									...
									<label className="vf-field">
										...
										{tplLocation.trim() !== "Templates" && (
											<small className="vf-field-hint">
												Templates outside <code>Templates/</code> won't
												show up in the New Workspace gallery.
											</small>
										)}
									</label>

									<p className="vf-export-preview">
										Captures taxonomy, views, dashboards and the people
										roster — no tasks or projects.
									</p>
