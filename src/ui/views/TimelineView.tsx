...
export function TimelineView({
	...
	const clientPointToDate = useCallback(
		(x: number, y: number): string | null => {
			...
			return isoFromDay(
				Math.max(domain.minDay, Math.min(domain.maxDay, day)),
			);
		},
	...
	const fitLabels = useCallback(() => {
		...
		const region = inner.closest(".vf-timeline-chart-region") as HTMLElement | null;
		const cap = region
	...
	const fitLower = useCallback(() => {
		...
		if (!body) return;
		const region = lowerRef.current?.closest(".vf-timeline-body") as
			| HTMLElement
			| null;
		const cap = region
				...
				{hasLower && (
					...
					>
						...
						>
							...
							<span className="vf-timeline-lower-hint">drag onto the chart to schedule</span>
						</button>
	...
	if (dayStep > 0) {
		...
		for (let m = startOfMonth(minDay); m < maxDay; m = addMonths(m, 1)) {
			...
			ticks.push({
				...
				left: (from - minDay) * scale,
				width: w,
				label: w > 22 ? MONTHS[date.getUTCMonth()] : MONTHS[date.getUTCMonth()][0],
