...
export function tabWorkspaceRoot(
  ...
  switch (tab.kind) {
    ...
    case "view":
      ...
      return plugin.index.snapshotWithView(tab.viewId)?.workspace.root ?? null;
    case "dashboard":
      return (
        plugin.index.snapshotWithDashboard(tab.dashboardId)?.workspace.root ??
        null
      );
    case "label":
      return (
        plugin.index.snapshotWithLabel(tab.labelId)?.workspace.root ?? null
      );
    case "person":
      return (
        plugin.index.snapshotWithPerson(tab.personId)?.workspace.root ?? null
      );
    case "query":
      return tab.root;
    default:
...
export interface TabsApi {
   ...
   * Keyed by tab id.
   */
  getSelectionSnapshot: (tabId: string) => {
...
export function TabsProvider({ children }: { children: ReactNode }) {
  ...
  const [selectionSnapshots, setSelectionSnapshots] = useState<
    Record<
      string,
      { focusedPath: string | null; selectedPaths: string[]; scrollTop: number }
    >
  >({});
  ...
  const getSelectionSnapshot = useCallback(
    (
      ...
      selectedPaths: string[];
      scrollTop: number;
    } | null => selectionSnapshotsRef.current[tabId] ?? null,
    [],
  );
  ...
  const getViewDraft = useCallback(
    (viewId: string): SavedView | null => viewDraftsRef.current[viewId] ?? null,
    [],
  );
    ...
    (root: string, definition: ViewDefinition, name: string) => {
      void (async () => {
        ...
        setTabs((current) =>
          current.some((tab) => tab.id === id)
            ? current
            : [...current, { id, kind: "query", root, name, definition }],
        );
        setActiveId(id);
      })();
    },
  ...
  const syncToWorkspace = useCallback(
    (root: string) => {
      ...
      const fallback =
        tabsRef.current.find((tab) => tabWorkspaceRoot(plugin, tab) === root) ??
        tabsRef.current.find((tab) => tabWorkspaceRoot(plugin, tab) == null);
      if (fallback) {
  ...
  const closeSubset = useCallback(
    async (keep: (tab: Tab) => boolean) => {
      ...
      if (activeClosing && !(await mayLeaveActive("close", activeId))) return;
    ...
    <T extends Tab["kind"]>(
      ...
      setTabs((current) => {
        const next = current.filter(
          (tab) => tab.kind !== kind || keep(tab as Extract<Tab, { kind: T }>),
        );
    ...
    (existing: (viewId: string) => boolean) =>
      ...
      makePrune("view", (tab) =>
        tab.root != null && isSystemViewId(tab.viewId)
          ? plugin.index.list().some((s) => s.workspace.root === tab.root)
          : existing(tab.viewId) || viewDraftsRef.current[tab.viewId] != null,
      ),
    [makePrune, plugin],
  );
