 ...
 * rather than a raw topic ID string, so a help-document rename fails the unit
 * test instead of silently producing a dead link at runtime.
 */

import { CircleHelp } from "lucide-react";
import { findHelpTopic, HELP_TOPICS } from "../../core/help";
import { HELP_TOPIC } from "../../core/help-links";
...
export function InlineHelpIcon({
  ...
  /** Accessible name; also the native tooltip when hovering the icon. */
  label: string;
  className?: string;
  size?: number;
}) {
  const { openHelp } = useTabs();
  const spec = HELP_TOPIC[target];
  const topic = findHelpTopic(HELP_TOPICS, spec.topicId);
  const title = topic?.title ?? spec.topicId;
