...
/**
 * Not a vault note — relative links and embeds in help content resolve from
 * the vault root against this placeholder path.
 */
...
function firstContentTopic(topics: readonly HelpTopic[]): HelpTopic | null {
 ...
 * instead of sitting invisibly inside collapsed parents.
 */
...
/**
 * The ancestor topics of `topicId`, ordered root first and closest parent
 * last, for the breadcrumb trail. A top-level topic has no breadcrumbs.
 */
function ancestorTopics(
  topics: readonly HelpTopic[],
  topicId: string,
): HelpTopic[] {
...
export function HelpView() {
   ...
   * This remains local until the relevant heading is present in the rendered
   * DOM; the TabsProvider copy is consumed on first read.
   */
  ...
  /**
   * A deep link via `openHelp` can arrive while the Help tab is already open.
   * Consume the provider target and align the local selected topic to it.
   */
  useEffect(() => {
    const target = pendingHelpTarget;
    if (!target) return;
  ...
  /**
   * MarkdownRenderer injects rendered headings asynchronously after React
   * commits. Watch the container until the requested heading is available,
   * then consume the pending anchor.
   */
  ...
  useEffect(() => {
    ...
    const tryScroll = (): boolean => {
      ...
      for (const heading of headings) {
        if (slugifyHeading(heading.textContent ?? "") !== pendingAnchor) {
          continue;
        }
  ...
  /**
   * In-content cross-links such as `[Layouts](help://layouts)` are rendered
   * as ordinary anchors by Obsidian's MarkdownRenderer. Intercept them during
   * bubbling and navigate inside the Help pane rather than leaving it.
   */
  const navigateFromLink: MouseEventHandler<HTMLDivElement> = (event) => {
    ...
    const anchor = target.closest<HTMLAnchorElement>(
      `a[href^="${HELP_LINK_PREFIX}"]`,
    );

    if (!anchor) return;

    const linkTarget = parseHelpLink(anchor.getAttribute("href") ?? "");
    if (!linkTarget) return;

    const topic = findHelpTopic(HELP_TOPICS, linkTarget.topicId);
    if (!topic) return;
    ...
    if (linkTarget.anchor) {
      setPendingAnchor(linkTarget.anchor);
    }
  };
          ...
          <>
            <header className="vf-help-header">
              {breadcrumbs.length > 0 && (
                <nav className="vf-help-breadcrumbs" aria-label="Breadcrumb">
                  {breadcrumbs.map((crumb, index) => (
                    <span key={crumb.id} className="vf-help-crumb">
                      {index > 0 && (
                        <span className="vf-help-crumb-arrow">/</span>
                      )}
  ...
  return (
    <>
      <div className="vf-help-topic-row" style={{ paddingLeft: depth * 14 }}>
        <button
          type="button"
          className={`vf-help-topic-toggle${isOpen ? " is-open" : ""}${
            hasChildren ? "" : " is-spacer"
          }`}
          aria-label={
