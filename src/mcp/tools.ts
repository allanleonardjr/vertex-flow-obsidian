    ...
    help: [
      {
        ...ref,
        ...(anchor
          ? { vaultUri: buildVaultUri({ action: "help", topicId, anchor }) }
          : {}),
      },
    ],
  };
...
function matchWorkspace(
  ...
  const matches = deps.index
    .list()
    .filter((snapshot) =>
      snapshot.workspace.name.toLocaleLowerCase().includes(needle),
    );
...
export function createMcpServer(deps: McpDeps): McpServer {
  ...
  server.registerTool(
    ...
    async () =>
      ok(pageList(deps.index.list().map((snapshot) => workspaceRow(snapshot)))),
  );
  ...
  server.registerTool(
    ...
    async ({ workspace, query, showArchived }) => {
      ...
      return ok({
        ...scopeOf(resolved.snapshot),
        ...pageList(taskRows(resolved.snapshot, {}, me, showArchived === true)),
      });
    },
  );
  ...
  server.registerTool(
    ...
    async ({ workspace, taskId }) => {
      ...
      const task = resolved.snapshot.tasks.find(
        (t) => t.id.toUpperCase() === query.toUpperCase() || t.path === query,
      );
      if (!task) {
        return fail(
          errorPayload(
  ...
  server.registerTool(
    ...
    async ({ workspace, viewId }) => {
      const resolved = resolveWorkspace(workspace);
      if ("error" in resolved) return fail(resolved.error);
      const view = resolved.snapshot.views.find((v) => v.id === viewId.trim());
      if (!view) {
        return fail(
          errorPayload(
            "unknown-view",
  ...
  server.registerTool(
    "list_dashboards",
    {
      description: "List a workspace's dashboards — id, name and widget count.",
      inputSchema: {
        workspace: z
          .string()
          .optional()
    ...
    async ({ workspace, personId }) => {
      ...
      const person = resolved.snapshot.workspace.people.find(
        (p) =>
          ...
          (p.aliases ?? []).some((a) => a.toLocaleLowerCase() === haystack),
      );
      if (!person) {
        return fail(
          errorPayload(
    ...
    "count_tasks",
    {
      ...
      inputSchema: {
        ...
        query: z
          ...
          .optional()
          .describe(
            "Query-language filter text; omit to count all visible tasks.",
          ),
        showArchived: z
          .boolean()
          .optional()
    ...
    "count_projects",
    {
      ...
      inputSchema: {
        ...
        showArchived: z
          ...
          .optional()
          .describe(
            "Count archived projects too (they're excluded from `total` otherwise).",
          ),
        byStatus: z
          .boolean()
          .optional()
  ...
  server.registerTool(
    "run_view",
    {
      ...
      inputSchema: {
        ...
        view: z.string().describe("The saved view's id, or its name."),
        withGroups: z
          .boolean()
          .optional()
          .describe(
            "Also return the grouped board/calendar/timeline breakdown.",
          ),
      },
    },
  ...
  server.registerTool(
    ...
    async ({ workspace }) => {
      ...
      const detail = {
        ...
        indexedNotePaths:
          resolved.snapshot.tasks.length + resolved.snapshot.projects.length,
        issues: {
  ...
  server.registerTool(
    "search_descriptions",
    {
      description:
        ...
        "regex (" +
        `\`${"..."}\`` +
        " described in the query-language " +
    ...
    async ({ workspace, text, project }) => {
      ...
      if (!needle) {
        return fail(errorPayload("invalid-query", "`text` must not be empty."));
      }
  ...
  server.registerTool(
    "search_help_docs",
    {
      ...
      inputSchema: {
        query: z.string().describe("A free-text description of what you need."),
      },
    },
    async ({ query }) => {
  ...
  server.registerTool(
    ...
    async ({ topicId }) => {
      ...
      if (!topic) {
        return fail(
          errorPayload("unknown-topic", `No help topic "${topicId}" exists.`),
        );
      }
